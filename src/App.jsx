...
function UpKeep({ data, setData }) {
  ...
  const upkeepItems = [
    ...
    { key: "last_chamber", label: "Chamber Wipe", frequency: 180, icon: Sparkles, firearms: firearms.filter(f => !f.last_cleaned || daysBetween(f.last_cleaned, today()) > 180) },
    ...
    <div className="tab">
      {upkeepItems.map(item => (
        <div key={item.key} style={{marginBottom: 24}}>
          <h3 style={{fontFamily: "'Oswald',sans-serif", fontSize: 16, marginBottom: 12, display: "flex", alignItems: "center", gap: 8}}><item.icon size={18} style={{color: "var(--accent)"}} /> {item.label} <span style={{color: "var(--faint)", fontSize: 12, marginLeft: "auto"}}>({item.firearms.length})</span></h3>
            ...
            <div style={{display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 12}}>
              {item.firearms.map(f => (
                <div key={f.id} style={{background: "var(--panel)", border: "1px solid var(--line)", borderRadius: "var(--radius)", padding: 14}}>
                  <div style={{display: "flex", justifyContent: "space-between", alignItems: "start", marginBottom: 10}}><strong style={{fontSize: 14}}>{f.nickname || f.manufacturer} {f.model}</strong><button className="primary small" onClick={() => resetUpkeep(f.id, item.key)}><Check size={12} /> Reset</button></div>
