...
import {
  ...
  ScrollText, Warehouse, Loader
} from "lucide-react";

/* ============================================================
   THE GUN SHED — With Real Supabase Backend
   - Email auth with verification
   - Four tables synced to Postgres
   - Target photos in private storage bucket
...
// Reference data
const MANUFACTURERS = [
  "Glock", "Smith & Wesson", "Sig Sauer", "Ruger", "Colt", "Remington",
  "Springfield Armory", "Beretta", "CZ", "Heckler & Koch", "Winchester",
  "Mossberg", "Savage Arms", "Browning", "FN Herstal", "Walther",
  "Kimber", "Daniel Defense", "Aero Precision", "Palmetto State Armory",
  "Henry", "Marlin", "Benelli", "Tikka", "Bergara", "Other",
];
const CALIBERS = [
  "9mm", ".45 ACP", ".40 S&W", ".380 ACP", "10mm", ".22 LR", ".223 Rem",
  "5.56 NATO", ".308 Win", "7.62x39", "6.5 Creedmoor", ".300 BLK",
  ".30-06", ".270 Win", "12 Gauge", "20 Gauge", ".410 Bore", ".357 Mag",
  ".38 Special", "44 Mag", "Other",
];
...
const CHANGELOG = [
  {
    version: "1.2.0", date: "2026-05-15", tag: "current",
    title: "Live Supabase backend",
    changes: [
      { type: "added", text: "Real email authentication with verification." },
      { type: "added", text: "Multi-device sync — your inventory syncs across phone, tablet, and desktop." },
      { type: "changed", text: "Data now lives in Postgres with Row Level Security — only you can see your data." },
      { type: "changed", text: "Target photos upload to Supabase Storage, no longer embedded in the database." },
      { type: "removed", text: "Client-side encryption (replaced by Supabase encryption-at-rest + RLS)." },
    ],
  },
  {
    version: "1.1.0", date: "2026-05-14", tag: "",
    title: "The Gun Shed redesign",
    changes: [
      { type: "added", text: "Renamed application to The Gun Shed." },
      { type: "added", text: "Changelog tab to track all versions and revisions." },
      { type: "added", text: "Client-side AES-GCM encryption — inventory stored as ciphertext." },
      { type: "changed", text: "Maintenance & Usage reworked into Range Log with target photos." },
      { type: "changed", text: "Add buttons moved to the left side of every toolbar." },
      { type: "fixed", text: "Add button on Range Log tab now opens entry form correctly." },
    ],
  },
  {
    version: "1.0.0", date: "2026-05-13", tag: "",
    title: "Initial build",
    changes: [
      { type: "added", text: "Firearms, Maintenance, Accessories and Ammunition tabs." },
      { type: "added", text: "Dashboard with collection stats and maintenance alerts." },
      { type: "added", text: "Local email login, persistent storage, CSV/JSON export." },
      { type: "added", text: "Per-tab search and multi-field sorting." },
    ],
  },
];
...
function toCSV(rows) {
  ...
  const cols = Object.keys(rows[0]).filter((c) => c !== "photos" && c !== "photo_paths");
  ...
  const submit = async () => {
    ...
    try {
      if (mode === "signup") {
        ...
        if (error) {
          setBusy(false);
          return setErr(error.message || "Signup failed.");
        }
        ...
        if (error) {
          setBusy(false);
          return setErr(error.message || "Login failed.");
        }
        onAuth(data.user);
      }
    } catch (e) {
  ...
  return (
    <div className="login-wrap">
      ...
      <div className="login-card">
        ...
        <button className="primary big" onClick={submit} disabled={busy}>
          {busy ? "Working…" : mode === "signup" ? "Create Account" : "Log In"}
        </button>

        <div className="note">
          <Lock size={12} />
          <span>Your data is protected by Row Level Security. Only you can access your inventory.</span>
        </div>
      </div>
    </div>
  );
...
function Stat({ icon: Icon, label, value, accent }) {
  return (
    <div className="stat" style={accent ? { borderColor: accent } : {}}>
  ...
  return (
    <div className="toolbar">
      ...
      <div className="sort">
        ...
        <button className="dir" onClick={() => setSortDir((d) => (d === "asc" ? "desc" : "asc"))}>
          {sortDir === "asc" ? "↑" : "↓"}
        </button>
      </div>
      <div className="spacer" />
      <div className="menu-wrap">
        <button className="ghost"><Download size={15} /> Export <ChevronDown size={13} /></button>
        <div className="menu">
...
function Firearms({ data, setData, userId }) {
  ...
  const blank = {
    nickname: "", manufacturer: "Glock", model: "", serial: "", caliber: "9mm", type: "Pistol", acquired: today(), value: "", notes: ""
  };
  ...
  try {
    if (rec.id && rec.id.length > 20) {
      // Real UUID — update existing record
      const { nickname, manufacturer, model, serial, caliber, type, acquired, value, notes } = rec;
      await supabase.from("firearms").update({ nickname, manufacturer, model, serial, caliber, type, acquired, value, notes }).eq("id", rec.id);
    } else {
      // New record — insert with user_id
      const { data: inserted, error } = await supabase.from("firearms").insert([{ nickname: rec.nickname, manufacturer: rec.manufacturer, model: rec.model, serial: rec.serial, caliber: rec.caliber, type: rec.type, acquired: rec.acquired, value: rec.value, notes: rec.notes, user_id: userId }]).select();
    ...
    setData((d) => {
      ...
      return {
        ...d,
        firearms: exists ? d.firearms.map((x) => (x.id === rec.id ? rec : x)) : [...d.firearms, rec]
      };
    });
    setEditing(null);
  } catch (e) {
  ...
  const remove = async (id) => {
    ...
    try {
      ...
      setData((d) => ({
        ...d,
        firearms: d.firearms.filter((x) => x.id !== id),
        rangelog: d.rangelog.filter((m) => m.firearm_id !== id),
      }));
    } catch (e) {
  ...
  const roundCount = useCallback((fid) =>
    (data.rangelog || []).filter((m) => m.firearm_id === fid).reduce((s, m) => s + (Number(m.rounds) || 0), 0),
    [data.rangelog]);

  return (
    <div className="tab">
      <Toolbar {...t} placeholder="Search firearms — maker, model, serial, caliber…"
        ...
        onAdd={() => setEditing({ ...blank, id: uid() })}
        onExportCSV={() => download("firearms.csv", toCSV(rows), "text/csv")}
        onExportJSON={() => download("firearms.json", JSON.stringify(rows, null, 2), "application/json")}
      />
      {t.view.length === 0 ? (
        <Empty icon={Target} label="No firearms yet" hint="Click Add Firearm to log your first one." />
      ) : (
        <div className="cards">
          {t.view.map((f) => (
            <div className="card" key={f.id}>
              <div className="card-top">
                ...
                <div className="card-title">
                  <strong>{f.nickname || f.model || "Unnamed"}</strong>
              ...
              <div className="kv">
                ...
                <div><label>Caliber</label><span>{f.caliber}</span></div>
      ...
      {editing && (
        <Modal title={rows.some((x) => x.id === editing.id) ? "Edit Firearm" : "Add Firearm"} onClose={() => setEditing(null)}>
          <FirearmForm rec={editing} onSave={save} onCancel={() => setEditing(null)} saving={saving} />
        </Modal>
      )}
    </div>
  );
...
function FirearmForm({ rec, onSave, onCancel, saving }) {
  ...
  return (
    <div className="form">
      <div className="grid2">
        ...
        <Field label="Type">
          <select value={f.type} onChange={(e) => set("type", e.target.value)}>{FIREARM_TYPES.map((x) => <option key={x}>{x}</option>)}</select>
        </Field>
        <Field label="Manufacturer">
          <select value={f.manufacturer} onChange={(e) => set("manufacturer", e.target.value)}>{MANUFACTURERS.map((x) => <option key={x}>{x}</option>)}</select>
        </Field>
        ...
        <Field label="Caliber / Gauge">
          <select value={f.caliber} onChange={(e) => set("caliber", e.target.value)}>{CALIBERS.map((x) => <option key={x}>{x}</option>)}</select>
        </Field>
      ...
      <div className="form-actions">
        ...
        <button className="primary" onClick={() => onSave(f)} disabled={!f.model && !f.nickname || saving}><Check size={15} /> {saving ? "Saving…" : "Save"}</button>
      </div>
    </div>
  );
}
...
function RangeLog({ data, setData, userId }) {
  ...
  const enriched = useMemo(() =>
    rows.map((m) => ({ ...m, firearm: fireName(m.firearm_id) })),
    [rows, data.firearms]);

  const t = useTable(enriched, ["firearm", "range_name", "notes"], "visit_date");

  const blank = {
    firearm_id: (data.firearms?.[0]?.id || ""),
    visit_date: today(),
    range_name: "",
    rounds: "",
    notes: "",
  ...
  const save = async (rec) => {
    ...
    try {
      const payload = {
        firearm_id: rec.firearm_id,
        visit_date: rec.visit_date,
        range_name: rec.range_name,
        rounds: rec.rounds ? parseInt(rec.rounds) : null,
        notes: rec.notes,
  ...
  await supabase.from("range_log").update({ visit_date, range_name, rounds, notes, photo_paths }).eq("id", rec.id);
      ...
      setData((d) => {
        ...
        return {
          ...d,
          rangelog: exists
            ? d.rangelog.map((x) => (x.id === rec.id ? { ...x, ...payload, id: rec.id } : x))
            : [...d.rangelog, { ...payload, id: rec.id }],
        };
      });
      setEditing(null);
    } catch (e) {
  ...
  return (
    <div className="tab">
      <Toolbar {...t} placeholder="Search range log — firearm, range, notes…"
        addLabel="Add Range Visit"
        sortOptions={[
        ...
        onAdd={() => data.firearms?.length ? setEditing({ ...blank, id: uid() }) : alert("Add a firearm first.")}
        onExportCSV={() => download("rangelog.csv", toCSV(enriched), "text/csv")}
        onExportJSON={() => download("rangelog.json", JSON.stringify(enriched, null, 2), "application/json")}
      />
      {t.view.length === 0 ? (
        <Empty icon={MapPin} label="No range visits logged" hint="Log each range trip — location, rounds fired, and target photos." />
      ) : (
        <div className="cards">
          {t.view.map((m) => (
            <div className="card" key={m.id}>
              <div className="card-top">
                <div className="card-ico range"><MapPin size={18} /></div>
                <div className="card-title">
                  <strong>{m.range_name || "Range visit"}</strong>
                  <span>{m.firearm}</span>
                </div>
                <span className="pill">{m.visit_date}</span>
              </div>
              <div className="kv">
              ...
              <div className="card-actions">
                <button className="icon-btn" onClick={() => setEditing(rows.find((x) => x.id === m.id))}><Edit3 size={15} /></button>
                <button className="icon-btn danger" onClick={() => remove(m.id)}><Trash2 size={15} /></button>
              </div>
            </div>
          ))}
        </div>
...
function RangeForm({ rec, firearms, userId, onSave, onCancel, saving }) {
    ...
    try {
      for (const file of Array.from(files)) {
        if (file.size > 2_500_000) {
          alert(`"${file.name}" is over 2.5 MB and was skipped.`);
          continue;
        }
  ...
  return (
    <div className="form">
      <div className="grid2">
        <Field label="Firearm">
          <select value={m.firearm_id} onChange={(e) => set("firearm_id", e.target.value)}>
            {firearms.map((f) => <option key={f.id} value={f.id}>{f.nickname || `${f.manufacturer} ${f.model}`}</option>)}
          </select>
        </Field>
      ...
      <Field label="Notes">
        <textarea rows={2} value={m.notes} onChange={(e) => set("notes", e.target.value)} placeholder="Drills run, zero, malfunctions, how it shot…" />
      </Field>
      <div className="form-fld">
        <span>Target photos</span>
        <label className="upload">
          <ImageIcon size={15} />
          <span>{uploadingImg ? "Uploading…" : "Add target photos"}</span>
            ...
            {m.photo_paths.map((path, i) => {
              ...
              return (
                <div className="thumb" key={i}>
                  <img src={url} alt={`Target ${i + 1}`} />
                  <button onClick={() => removePhoto(i)}><X size={12} /></button>
                </div>
              );
            })}
          </div>
      ...
      <div className="form-actions">
        ...
        <button className="primary" onClick={() => onSave(m)} disabled={saving || uploadingImg}>
...
function Accessories({ data, setData, userId }) {
  ...
  await supabase.from("accessories").update({ name, type, brand, quantity, assigned_to, value, notes }).eq("id", rec.id);
} else {
        const { data: inserted, error } = await supabase.from("accessories").insert([{ name: rec.name, type: rec.type, brand: rec.brand, quantity: parseInt(rec.quantity) || 1, assigned_to: rec.assigned_to, value: rec.value ? parseFloat(rec.value) : null, notes: rec.notes, user_id: userId }]).select();
        if (inserted?.length) rec = inserted[0];
      }
      setData((d) => {
        ...
        return {
          ...d,
          accessories: exists ? d.accessories.map((x) => (x.id === rec.id ? rec : x)) : [...d.accessories, rec]
        };
      });
      setEditing(null);
    } catch (e) {
  ...
  return (
    <div className="tab">
      <Toolbar {...t} placeholder="Search accessories — name, type, brand…"
        ...
        onAdd={() => setEditing({ ...blank, id: uid() })}
        onExportCSV={() => download("accessories.csv", toCSV(rows), "text/csv")}
        onExportJSON={() => download("accessories.json", JSON.stringify(rows, null, 2), "application/json")}
      />
      {t.view.length === 0 ? (
        <Empty icon={Package} label="No accessories yet" hint="Scopes, holsters, grips, magazines and more." />
      ) : (
        <div className="cards">
          {t.view.map((a) => (
            <div className="card" key={a.id}>
              <div className="card-top">
                <div className="card-ico acc"><Package size={18} /></div>
                <div className="card-title">
              ...
              <div className="kv">
                <div><label>Quantity</label><span>{a.quantity}</span></div>
                <div><label>Assigned to</label><span>{a.assigned_to || "Unassigned"}</span></div>
                <div><label>Est. value</label><span>{money(a.value)}</span></div>
              </div>
        ...
        <Modal title={rows.some((x) => x.id === editing.id) ? "Edit Accessory" : "Add Accessory"} onClose={() => setEditing(null)}>
          <div className="form">
            <div className="grid2">
              ...
              <Field label="Type">
                <select value={editing.type} onChange={(e) => setEditing({ ...editing, type: e.target.value })}>{ACCESSORY_TYPES.map((x) => <option key={x}>{x}</option>)}</select>
              </Field>
              ...
              <Field label="Assigned to firearm">
                <select value={editing.assigned_to} onChange={(e) => setEditing({ ...editing, assigned_to: e.target.value })}>
                  <option value="">Unassigned</option>
                  {(data.firearms || []).map((f) => <option key={f.id} value={f.nickname || `${f.manufacturer} ${f.model}`}>{f.nickname || `${f.manufacturer} ${f.model}`}</option>)}
                </select>
              </Field>
            ...
            <div className="form-actions">
              <button className="ghost" onClick={() => setEditing(null)} disabled={saving}>Cancel</button>
              <button className="primary" onClick={() => save(editing)} disabled={saving}><Check size={15} /> {saving ? "Saving…" : "Save"}</button>
            </div>
          </div>
        </Modal>
      )}
...
function Ammo({ data, setData, userId }) {
  ...
  user_id: userId 
      ...
      setData((d) => {
        ...
        return {
          ...d,
          ammo: exists ? d.ammo.map((x) => (x.id === rec.id ? rec : x)) : [...d.ammo, rec]
        };
      });
      setEditing(null);
    } catch (e) {
  ...
  return (
    <div className="tab">
      <Toolbar {...t} placeholder="Search ammo — caliber, type, brand, location…"
        addLabel="Add Ammunition"
        sortOptions={[
          { key: "caliber", label: "Caliber" },
          { key: "type", label: "Type" },
          { key: "quantity", label: "Quantity" },
        ...
        onAdd={() => setEditing({ ...blank, id: uid() })}
        onExportCSV={() => download("ammo.csv", toCSV(rows), "text/csv")}
        onExportJSON={() => download("ammo.json", JSON.stringify(rows, null, 2), "application/json")}
      />
      {t.view.length === 0 ? (
        <Empty icon={Boxes} label="No ammunition logged" hint="Track type, quantity and storage location." />
      ) : (
        <table className="grid-table">
          <thead>
            <tr><th>Caliber</th><th>Type</th><th>Brand</th><th>Grain</th><th>Quantity</th><th>Location</th><th></th></tr>
          </thead>
          <tbody>
            {t.view.map((a) => (
              <tr key={a.id} className={Number(a.quantity) < LOW ? "low" : ""}>
                <td><strong>{a.caliber}</strong></td>
                <td>{a.type}</td>
                <td>{a.brand || "—"}</td>
                <td>{a.grain || "—"}</td>
                <td>
                  {Number(a.quantity).toLocaleString()}
                  {Number(a.quantity) < LOW && <span className="low-flag"><AlertTriangle size={12} /> low</span>}
                </td>
                <td className="dim">{a.location || "—"}</td>
                <td className="row-actions">
                  <button className="icon-btn" onClick={() => setEditing(a)}><Edit3 size={14} /></button>
      ...
      {editing && (
        <Modal title={rows.some((x) => x.id === editing.id) ? "Edit Ammunition" : "Add Ammunition"} onClose={() => setEditing(null)}>
          <div className="form">
            <div className="grid2">
              <Field label="Caliber / Gauge">
                <select value={editing.caliber} onChange={(e) => setEditing({ ...editing, caliber: e.target.value })}>{CALIBERS.map((x) => <option key={x}>{x}</option>)}</select>
              </Field>
              <Field label="Type">
                <select value={editing.type} onChange={(e) => setEditing({ ...editing, type: e.target.value })}>{AMMO_TYPES.map((x) => <option key={x}>{x}</option>)}</select>
              </Field>
        ...
        {CHANGELOG.map((rel) => (
          <div className="release" key={rel.version}>
            <div className="release-rail">
              <div className={`release-dot ${rel.tag === "current" ? "cur" : ""}`} />
            </div>
...
function Dashboard({ data, go }) {
  ...
  const collectionValue =
    (data.firearms || []).reduce((s, f) => s + (Number(f.value) || 0), 0) +
    (data.accessories || []).reduce((s, a) => s + (Number(a.value) || 0) * (Number(a.quantity) || 1), 0);

  const alerts = useMemo(() => {
    const out = [];
    (data.firearms || []).forEach((f) => {
      const logs = (data.rangelog || []).filter((m) => m.firearm_id === f.id).sort((a, b) => new Date(b.visit_date) - new Date(a.visit_date));
      const name = f.nickname || `${f.manufacturer} ${f.model}`;
      const rounds = logs.reduce((s, m) => s + (Number(m.rounds) || 0), 0);
      if (rounds >= 500) out.push({ name, msg: `${rounds.toLocaleString()} rounds logged — consider a detail clean & inspection.` });
      if (logs[0] && daysBetween(logs[0].visit_date, today()) > 180)
        out.push({ name, msg: `Last range visit was ${daysBetween(logs[0].visit_date, today())} days ago.` });
    });
    (data.ammo || []).forEach((a) => {
      if (Number(a.quantity) < 100) out.push({ name: `${a.caliber} ${a.type}`, msg: `Low stock — ${a.quantity} rounds left.` });
    });
    return out;
  }, [data]);
  ...
  return (
    <div className="tab">
      <div className="dash-stats">
        <Stat icon={Target} label="Firearms" value={(data.firearms || []).length} accent="#c8643c" />
        <Stat icon={MapPin} label="Range visits" value={(data.rangelog || []).length} accent="#5b8a72" />
        <Stat icon={Package} label="Accessories" value={(data.accessories || []).reduce((s, a) => s + (Number(a.quantity) || 1), 0)} accent="#7a86b8" />
        <Stat icon={Boxes} label="Rounds on hand" value={totalAmmo.toLocaleString()} accent="#b89a4a" />
        <Stat icon={Crosshair} label="Rounds fired" value={totalRounds.toLocaleString()} accent="#9c5a5a" />
        <Stat icon={Warehouse} label="Collection value" value={money(collectionValue)} accent="#6f9bb5" />
      </div>
      <div className="dash-grid">
        <div className="panel">
          <div className="panel-head"><AlertTriangle size={16} /><h3>Maintenance & Stock Alerts</h3></div>
          {alerts.length === 0 ? (
            <p className="panel-empty">Everything looks good. No alerts right now.</p>
          ) : (
            <ul className="alert-list">
              {alerts.map((a, i) => <li key={i}><strong>{a.name}</strong><span>{a.msg}</span></li>)}
            </ul>
          )}
        </div>
        <div className="panel">
          <div className="panel-head"><Calendar size={16} /><h3>Recent Range Visits</h3></div>
          {(data.rangelog || []).length === 0 ? (
            <p className="panel-empty">No range visits logged yet.</p>
          ) : (
            <ul className="activity-list">
              {[...(data.rangelog || [])].sort((a, b) => new Date(b.visit_date) - new Date(a.visit_date)).slice(0, 6).map((m) => {
      ...
      <div className="quick">
        <button onClick={() => go("firearms")}><Target size={15} /> Manage firearms</button>
        <button onClick={() => go("rangelog")}><MapPin size={15} /> Log a range visit</button>
        <button onClick={() => go("accessories")}><Package size={15} /> Manage accessories</button>
        <button onClick={() => go("ammo")}><Boxes size={15} /> Manage ammo</button>
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  if (!authChecked) return <div className="boot"><Loader size={20} style={{ animation: "spin 1s linear infinite" }} /> Loading…</div>;
  ...
  const SUB = {
    dashboard: "Overview of your collection, alerts and recent range visits.",
    firearms: "Identification, caliber, value and round count for each firearm.",
          ...
          <div className="content">
            {loading ? (
              <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "400px", color: "var(--dim)" }}>
                <Loader size={20} style={{ animation: "spin 1s linear infinite", marginRight: "10px" }} />
                Loading your data…
              </div>
            ) : (
              <>
...
function Styles() {
  return (
    ...
    * { box-sizing:border-box; margin:0; padding:0; }
    ...
    .app { display:flex; min-height:100vh; font-family:'Archivo',system-ui,sans-serif;
      color:var(--text);
    ...
    .brand { display:flex; align-items:center; gap:9px; color:var(--accent);
      font-family:'Oswald',sans-serif; font-weight:700; letter-spacing:2px; font-size:20px; }
    .brand.sm { font-size:16px; letter-spacing:1.5px; padding:4px 4px 0; }
    .brand span { color:var(--text); }

    .sidebar { width:248px; flex-shrink:0; background:var(--bg2); border-right:1px solid var(--line);
      padding:22px 16px; display:flex; flex-direction:column; gap:24px; position:sticky; top:0; height:100vh; }
    .sidebar nav { display:flex; flex-direction:column; gap:3px; }
    .sidebar nav button { display:flex; align-items:center; gap:11px; padding:11px 12px; background:transparent;
      border:none; border-radius:8px; cursor:pointer; color:var(--dim); font-family:'Archivo',sans-serif;
      font-size:13.5px; font-weight:500; text-align:left; transition:all .15s; width:100%; }
    .sidebar nav button:hover { background:var(--panel); color:var(--text); }
    .sidebar nav button.on { background:linear-gradient(90deg, rgba(200,100,60,0.16), rgba(200,100,60,0.04));
      color:var(--text); box-shadow:inset 2px 0 0 var(--accent); }
    .side-foot { margin-top:auto; display:flex; flex-direction:column; gap:10px; }
    .lock-badge { display:flex; align-items:center; gap:6px; font-size:11px; color:var(--green);
      background:rgba(91,138,114,0.1); border:1px solid rgba(91,138,114,0.3); padding:6px 10px;
      border-radius:7px; justify-content:center; }
    .user { display:flex; align-items:center; gap:9px; padding:8px; background:var(--panel);
      border:1px solid var(--line); border-radius:8px; }
    .avatar { width:30px; height:30px; border-radius:7px; background:var(--accent); color:#fff;
      display:grid; place-items:center; font-weight:700; font-size:14px; flex-shrink:0; }
    .user .email { font-size:11.5px; color:var(--dim); overflow:hidden; text-overflow:ellipsis;
    ...
    .search { display:flex; align-items:center; gap:8px; background:var(--panel); border:1px solid var(--line);
      border-radius:8px; padding:0 11px; flex:1; min-width:220px; color:var(--faint); }
    .search input { background:transparent; border:none; outline:none; color:var(--text);
      font-family:inherit; font-size:13px; padding:9px 0; width:100%; }
    .sort { display:flex; align-items:center; gap:6px; background:var(--panel); border:1px solid var(--line);
      border-radius:8px; padding:0 9px; color:var(--faint); }
    .sort select { background:transparent; border:none; outline:none; color:var(--text); font-family:inherit;
      font-size:12.5px; padding:9px 2px; cursor:pointer; }
    .sort select option { background:var(--panel2); }
    .sort .dir { background:var(--panel2); border:1px solid var(--line); color:var(--text); width:24px;
      height:24px; border-radius:6px; cursor:pointer; font-size:13px; }
    .spacer { flex:1; }

    button.primary { display:flex; align-items:center; gap:6px; background:var(--accent); color:#fff;
      border:none; border-radius:8px; padding:9px 15px; cursor:pointer; font-family:inherit; font-size:13px;
    ...
    button.ghost { display:flex; align-items:center; gap:6px; background:var(--panel); border:1px solid var(--line);
      color:var(--dim); border-radius:8px; padding:9px 13px; cursor:pointer; font-family:inherit; font-size:12.5px;
      transition:all .15s; }
    button.ghost:hover { color:var(--text); border-color:var(--line2); }
    button.ghost:disabled { opacity:.5; cursor:not-allowed; }

    .menu-wrap { position:relative; }
    .menu-wrap .menu { display:none; position:absolute; right:0; top:110%; background:var(--panel2);
      border:1px solid var(--line2); border-radius:8px; overflow:hidden; z-index:20; min-width:150px;
    ...
    .menu button { display:block; width:100%; text-align:left; background:transparent; border:none;
      color:var(--dim); padding:10px 13px; font-family:inherit; font-size:12.5px; cursor:pointer; }
    .menu button:hover { background:var(--panel); color:var(--text); }

    .icon-btn { background:var(--panel2); border:1px solid var(--line); color:var(--dim); width:30px;
      height:30px; border-radius:7px; cursor:pointer; display:grid; place-items:center; transition:all .15s; }
    ...
    .card-title strong { display:block; font-size:14.5px; font-family:'Oswald',sans-serif; font-weight:600;
      letter-spacing:.3px; }
    .card-title span { font-size:12px; color:var(--dim); }
    .pill { font-size:10.5px; text-transform:uppercase; letter-spacing:.6px; background:var(--panel2);
      border:1px solid var(--line2); color:var(--dim); padding:3px 8px; border-radius:20px; flex-shrink:0; }
    ...
    .thumbs img { width:54px; height:54px; object-fit:cover; border-radius:6px; border:1px solid var(--line2);
    ...
    .thumb button { position:absolute; top:-6px; right:-6px; width:18px; height:18px; border-radius:50%;
      background:var(--danger); color:#fff; border:none; cursor:pointer; display:grid; place-items:center;
      padding:0; }
    .upload { display:inline-flex; align-items:center; gap:7px; background:var(--panel); border:1px dashed var(--line2);
      color:var(--dim); padding:9px 13px; border-radius:8px; cursor:pointer; font-size:12.5px; width:fit-content;
      transition:all .15s; }
    .upload:hover { color:var(--text); border-color:var(--accent); }

    .grid-table { width:100%; border-collapse:collapse; background:var(--panel); border:1px solid var(--line);
      border-radius:var(--radius); overflow:hidden; }
    .grid-table th { text-align:left; font-size:10.5px; text-transform:uppercase; letter-spacing:.7px;
      color:var(--faint); padding:11px 14px; background:var(--bg2); border-bottom:1px solid var(--line); }
    .grid-table td { padding:12px 14px; font-size:13px; border-bottom:1px solid var(--line); }
    ...
    .low-flag { display:inline-flex; align-items:center; gap:3px; margin-left:8px; font-size:10.5px;
      color:var(--danger); text-transform:uppercase; letter-spacing:.5px; }

    .empty { display:flex; flex-direction:column; align-items:center; gap:8px; padding:70px 20px;
      color:var(--faint); text-align:center; border:1px dashed var(--line2); border-radius:var(--radius); }
    .empty strong { font-size:15px; color:var(--dim); font-family:'Oswald',sans-serif; }
    .empty span { font-size:12.5px; }

    .dash-stats { display:grid; grid-template-columns:repeat(auto-fill, minmax(190px, 1fr)); gap:13px;
      margin-bottom:22px; }
    .stat { display:flex; align-items:center; gap:13px; background:var(--panel); border:1px solid var(--line);
      border-left-width:3px; border-radius:var(--radius); padding:15px 16px; }
    ...
    .panel-head h3 { font-family:'Oswald',sans-serif; font-size:15px; font-weight:600; color:var(--text);
      letter-spacing:.4px; }
    ...
    .alert-list li { display:flex; flex-direction:column; gap:2px; padding:10px 12px;
      background:rgba(184,80,74,0.08); border:1px solid rgba(184,80,74,0.25); border-radius:7px; }
    ...
    .activity-list li { display:grid; grid-template-columns:90px 1fr 1fr; gap:8px; padding:9px 0;
      border-bottom:1px solid var(--line); font-size:12.5px; }
    .activity-list li:last-child { border-bottom:none; }
    .quick { display:flex; gap:10px; flex-wrap:wrap; }
    .quick button { display:flex; align-items:center; gap:7px; background:var(--panel); border:1px solid var(--line);
      color:var(--dim); padding:11px 16px; border-radius:8px; cursor:pointer; font-family:inherit; font-size:12.5px;
      transition:all .15s; }
    ...
    .release-dot { width:12px; height:12px; border-radius:50%; background:var(--panel2);
      border:2px solid var(--line2); margin-top:4px; z-index:1; }
    ...
    .release-head .ver { font-family:'Oswald',sans-serif; font-weight:700; font-size:13px; color:var(--accent);
      background:rgba(200,100,60,0.1); border:1px solid rgba(200,100,60,0.3); padding:2px 9px; border-radius:6px; }
    .release-head h3 { font-family:'Oswald',sans-serif; font-size:18px; font-weight:600; }
    .cur-badge { font-size:10px; text-transform:uppercase; letter-spacing:.6px; background:var(--green);
    ...
    .change-list { list-style:none; display:flex; flex-direction:column; gap:8px; background:var(--panel);
      border:1px solid var(--line); border-radius:var(--radius); padding:14px 16px; }
    .change-list li { display:flex; gap:10px; align-items:flex-start; font-size:13px; line-height:1.5; }
    .ct { font-size:9.5px; text-transform:uppercase; letter-spacing:.6px; font-weight:700; padding:3px 7px;
      border-radius:5px; flex-shrink:0; margin-top:1px; min-width:54px; text-align:center; }
    ...
    .modal-back { position:fixed; inset:0; background:rgba(10,9,6,0.78); display:grid; place-items:center;
      z-index:100; padding:20px; backdrop-filter:blur(2px); }
    .modal { background:var(--bg2); border:1px solid var(--line2); border-radius:14px; width:100%;
    ...
    .form-fld input, .form-fld select, .form-fld textarea { background:var(--panel); border:1px solid var(--line);
      border-radius:7px; padding:9px 11px; color:var(--text); font-family:inherit; font-size:13px; outline:none;
    ...
    .login-wrap { min-height:100vh; display:grid; place-items:center; position:relative;
      font-family:'Archivo',sans-serif; padding:20px; overflow:hidden; }
    .login-bg { position:absolute; inset:0; background:
      radial-gradient(700px 400px at 20% 20%, rgba(200,100,60,0.12), transparent 60%),
      radial-gradient(600px 400px at 90% 90%, rgba(91,138,114,0.10), transparent 60%), var(--bg);
      z-index:0; }
    .login-card { position:relative; z-index:1; background:var(--bg2); border:1px solid var(--line2);
      border-radius:16px; padding:34px 30px; width:100%; max-width:380px; box-shadow:0 30px 70px rgba(0,0,0,0.5); }
    ...
    .seg { display:flex; background:var(--panel); border:1px solid var(--line); border-radius:9px; padding:3px;
      margin-bottom:18px; }
    .seg button { flex:1; background:transparent; border:none; color:var(--dim); padding:8px; border-radius:6px;
      cursor:pointer; font-family:inherit; font-size:12.5px; font-weight:600; transition:all .15s; }
    ...
    .fld input { background:var(--panel); border:1px solid var(--line); border-radius:8px; padding:10px 12px;
      color:var(--text); font-family:inherit; font-size:13.5px; outline:none; transition:border-color .15s; }
    .fld input:focus { border-color:var(--accent); }
    .err { display:flex; align-items:center; gap:6px; background:rgba(184,80,74,0.12);
      border:1px solid rgba(184,80,74,0.35); color:#d98a84; font-size:12px; padding:8px 11px; border-radius:7px;
      margin-bottom:12px; }
